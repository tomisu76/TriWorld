...
def create_synthetic_canary(output_zip: Path, level_name: str = "synthetic_canary") -> Dict[str, str]:
    ...
    Contents:
    - Flat terrain (256x256, 2m resolution)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 2. Create terrain metadata .terrain.json
        terrain_json = {
            "version": 1,
        ...
        # 5. Create main.materials.json
        materials_json = {
            "materials": [
                {
                    "name": "asphalt",
                    "diffuseMap": "art/shapes/roads/asphalt_diffuse.png",
                    "normalMap": "art/shapes/roads/asphalt_normal.png",
                    "specularMap": "art/shapes/roads/asphalt_specular.png",
                    "materialTag": "Road",
                    "friction": 0.9,
                    "restitution": 0.1,
                    "sound": "asphalt",
                ...
                {
                    "name": "grass",
                    "diffuseMap": "art/shapes/terrain/grass_diffuse.png",
                    "normalMap": "art/shapes/terrain/grass_normal.png",
                    "materialTag": "Grass",
                    "friction": 0.7,
                    "restitution": 0.1,
                    "sound": "grass",
        ...
        # 6. Create items.level.json (LDJSON)
        items_lines = []
        ...
        # DecalRoad nodes: [x, y, z, halfWidth]
        half_width = 3.5
        decal_nodes = [
            [0.0, 0.0, 0.01, half_width],
            [500.0, 0.0, 0.01, half_width],
        ]
        items_lines.append(json.dumps({
            "class": "DecalRoad",
            "name": "ai_road_001",
        ...
        # 7. Create environment items.level.json
        env_items = [
        ...
        # 11. Create reports/build-manifest.json
        ...
        (level_dir / "README.txt").write_text(
            ...
            f"\n"
            f"Contents:\n"
            f"- Flat terrain (256x256, 2m resolution)\n"
            f"- Straight road (500m x 7m)\n"
            f"- TSStatic physical road mesh\n"
            f"- AI DecalRoad centerline\n"
            f"- Single spawn point\n"
        )
        
        # 15. Create deterministic ZIP
        file_hashes = {}
        
        # Collect all files with deterministic ordering (sorted by POSIX path string)
        all_files = []
        for f in sorted(staging.rglob('*'), key=lambda p: p.relative_to(staging).as_posix()):
            if f.is_file():
                rel = f.relative_to(staging)
        ...
        # Re-write build-manifest.json with the final sorted file list
        manifest = {
            "buildId": hashlib.sha256(json.dumps({"name": level_name}, sort_keys=True).encode()).hexdigest()[:16],
            "compilerVersion": "0.1.0",
            "requestHash": "synthetic_canary",
        ...
        # Write ZIP with deterministic ordering and fixed timestamps
        # Use a fixed timestamp (1980-01-01 00:00:00) for all entries
